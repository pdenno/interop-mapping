...
Entity IDs are internal to the database and not of much value to RADmapper users.\footnote{In fact, I sort of told a fib for ease of exposition; by default the binding of variables in the entity position, entity IDs, aren't provided in a binding object.
  What this example actually returns is \stt{[\{\}]} meaning ``one match was found.''}
...
In this example, we used a variable to match on the entity position, however each position (entity, attribute, and value) can take a variable.
...
\stt{query\{[\_ ?attr \_]\}} would return all the names of all the attributes in the database (without duplicates). % ToDo: Fix this in the code!
...
You have probably noticed that the query above, and one used earlier have four elements in their pattern whereas most of the examples have only three (entity, attribute and value).
...
Line 12 calls the query function bound to \stt{\$bSets} to get the binding sets against the two databases.
...
The binding set that is produced, the value of \stt{\$bSets}, consists of two binding objects:
...
% Oops! Save this for later!
%{"alice@alice.org" {"aData" "Alice-A-data", "bData" "Alice-B-data", "email" "alice@alice.org", "name" "Alice"},
% "bob@example.com" {"aData" "Bob-A-data", "bData" "Bob-B-data", "email" "bob@example.com", "name" "Bob"}}
...
The key difference is that on Line 4, the \stt{query} construct defines a parameter using ordinary JSONata-like syntax.
You can define as many parameters as you'd like and they can be used to substitute into any of the pattern positions, entity, attribute for value.
By convention if we are to assign the result to a variable we end the variable with a \stt{T} such as shown on Line 4, \stt{\$ageQueryT}.
...
Binding sets produced by \stt{query} provide ordinary JSONata-like objects that could be used with JSONata built-in functions and operators to produce target structures.\footnote{Two differences: (1) the keys of binding sets are query variables, and (2) binding sets are flat objects; the values at the keys are not objects themselves. These differences notwithstanding, you can use binding sets as though they are JSONata objects.}
...
We'd like to \textbf{[....ToDo]}.

\subsection{Predicate patterns and an example with a different perspective}
...
Though these structures, rendered as JSON, may look odd (for example, there are no arrays used despite apparent value in having them\footnote{For example, several devices are associated with an owner. Instead of arrays, the developer used meaningful names as keys, "device1","device2" etc. Perhaps these structures only seems unusual if your perspective is objects indexed by named fields. }) this is not relevant to the discussion.
...
This example is the first to use extensive joins in a ``hairpin'' pattern, as depicted in \ref{fig:hairpin}.
