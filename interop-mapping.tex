...
The mapping language  described (provisionally called RADmapper) borrows predominantly from the JSONata mapping language~\cite{Jsonata.org2021}, but also includes provisions for mapping to/from forms other than JSON.\@
...
Whether or not the tool is useful in operational settings remains to be seen;
we'll generate an OpenAPI Specification for it to test that idea.
At the current state of development, it isn't clear whether the language being described is the interoperable exchange form itself or something derived from what is being developed.
...
For example, without additional knowledge about the data, how do we know
(a) that an action is intended to update some data rather than add new additional information about the object, and
