...
% ToDo: Investigate optional arguments or another way to do this.
...
\begin{figure}[H]
  \caption{An object (\stt{owl/Class}) and a relation (\stt{owl/ObjectProperty}) in the source population.
...
We'll assume that we want to support storage of individuals of the types defined by OWL classes and that all relations are conceptually binary.
Thus for each \stt{owl/ObjectProperty} we will produce a two-column table to represent both a relation and its inverse (``inverse pairs'') where an inverse is defined.
Note that, owing to the inverse pairs choice, we'll have to prevent creating a table for one member of each inverse pair. % Great use of a WHERE (or something!) Maybe I'll have TWO different mapping actions, one where there is an inverse, and one where there is not.
We'll use \stt{resource/iri} as the keys of these tables. 
To keep things simple, we'll omit some otherwise useful considerations like identifying what columns define keys; we'll assume \stt{rdfs/domain}
and \stt{rdfs/range} are single-valued.
...
%
For example, were the only data processed the data in Figure~\label{code:endurant}, the following might suffice.

\begin{figure}[H]
  \caption{Result of mapping data from Figure~\label{code:endurant}.
    This relational design captures the OWL 'meta-data' and provides for storage of individuals.}
  \label{code:table-instances}
\begin{lstlisting}[numberstyle=\scriptsize,basicstyle=\ttfamily\scriptsize,numbers=left,stepnumber=1,breaklines=true]
...
% u20\subsection{Table of language features for use with collections}
% \begin{table}[H]
% \begin{tabular}{l | l }
% \textbf{Task}                       & \textbf{Example}               \\ \hhline{=|=}
