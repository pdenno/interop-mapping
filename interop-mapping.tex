...
\begin{figure}[H]
  \caption{Result of mapping data depicted in Figure~\ref{code:endurant}.
    This relational design captures the OWL 'meta-data' and provides for storage of individuals.
    Therefore, it specifies content equivalent to
    (1) data definition language for the class and relation definitions, and
    (2) data manipulation language for creating the specific class \stt{dol/perdurant} and the specific relation pair
    \stt{(dol/participant, dol/participant-in)}}.
  \label{code:table-instances}
\begin{lstlisting}[numberstyle=\scriptsize,basicstyle=\ttfamily\scriptsize,numbers=left,stepnumber=1,breaklines=true]
  {'instance-of'  : 'schema',
   'schema/tables :  [{'instance-of'   : 'table-definition',
                       'table/name'    : 'object-definition-table',
                       'table/columns' : [{'instance-of'  : 'column-definition',
                                           'column/name'  : 'resource-iri',
                                           'column/dtype' : 'key-component'}
                                          {'instance-of'  : 'column-definition',
                                           'column/name'  : 'resource-label',
                                           'column/dtype' : 'string-data'}
                                          {'instance-of'  : 'column-definition',
                                           'column/name'  : 'resource-namespace',
                                           'column/dtype' : 'string-data'}]}

                       {'instance-of'   : 'table-definition',
                        'table/name'    : 'relation-definition-table',
                        'table/columns' : [{'instance-of'  : 'column-definition',
