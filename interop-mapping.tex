...
The section in curly brackets below the triples defines how values from the binding set are used in the target population.
...
On Line 1 we constrain the search to just those entities that have \stt{rdf/type} containing the string ``owl/Class''.
On Lines 2, 3, and 4 we bind variables to attribute values of that entity.
If instead of mapping every namespace in the source data we wanted to limit mapping to a single namespace,
we could have specified a value or expression for \stt{resource/namespace} on Line 3 rather than the variable \stt{?ns}.
...
Lines 5, 6, 7 and 8 match and bind a second entity, one that according to Line 5 must have \stt{rdf/type}
containing either ``owl/ObjectType'' or ``owl/DataType''.
Line 6 uses the variable \stt{?class-iri} which was also bound on Line 2; thus a relational join is performed;
we are looking for relations whose \stt{rdfs/domain} is the class bound in Lines 1 through 4.
...
That does not necessarily entail 500 structures like Lines 9 through 13, however.
