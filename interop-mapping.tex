...
Index variables are things such as \stt{i} that you find code and pseudocode such as \stt{for i in [1..9] \{\ldots \}}.
...
If the table is transposed (so that all the properties are in its first column), a third argument with value \stt{true} can be specified.
...
JSONata is used, for example, in NodeRed~\cite{Node-Red2021} the Open Integration Hub~\cite{OIH2021}, an open software platform that received its initial funding in 2017 from the German Federal Government,
...
Relational and graph-based data can be described by triples $[x,rel,y]$ where $x$ is an entity identifier, $y$ is data (string, number, object, array etc.)\footnote{What  additional atomic data
  types shall we provide? Dates? URIs? UUIDs? etc.}~\footnote{Triples can maintain the normalization inherent in the base form; in fact, the normalization to such triples can be 5th-normal form.}
...
for example, you can imagine \stt{[^^3fe "name" "Bob"]} being part of a query to get the entity identifier for the \stt{Bob} object (where \stt{^^3fe} is a \textit{query variable}).
...
If the datoms\footnote{Triples used in this context are sometimes called \textit{datoms}. I am glossing over some detail here. In fact, datoms in some implementations include a transaction time;
  data is never destroyed; instead, on query, you get the most recent value for a datom unless you specifically direct the query to look at the state of the DB at an earlier date.
  Such DB are sometimes called immutable databases. I mention this only in case someone thinks they might have a use for that functionality. If you do, let me know!} queried are just the ones of
Figure~\ref{code:bob-phone}, then the datom on Line 1 matches the query pattern \stt{[^^3fe "name" "Bob"]}, binding \stt{?e} to the entity id 13.
...
Such a structure is called a \textit{binding set}. 
...
There are certainly instances where that is useful, but is \stt{query} any easier than pure JSONata in these circumstances?\footnote{Stakeholder, do you have 
...
The unique value of \stt{query} is as an argument to higher-level functions to do tasks such as in-place and bi-directional updating.
Therefore, discussed next is (1) a new construct called \stt{enforce} and (2) how \stt{query} and \stt{enforce} can be used as arguments to a higher-order function \stt{transform}.
