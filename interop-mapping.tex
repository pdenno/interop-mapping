...
A reference implementation of the RADmapper language can be found in the Github respository \href{https://github.com/pdenno/RADmapper}{https://github.com/pdenno/RADmapper}. % href
...
This document only goes into detail about capabilities of RADmapper not found in JSONata.

\subsection{Data Organized as Triples}

The principal concepts to discuss are
(1) the relational and graph forms of data,
(2) RADmapper's \stt{query} declaration, used to query relational data, and
(3) RADmapper's \stt{express} declaration, used to reorganize (``map'') data from its original form to the form in which it is needed.
The original form of data is called a \textit{source} form; the needed form is called the \textit{target} form.

\stt{query} and \stt{express} are the principal constructs of RADmapper that provide Datalog-like functionality to the language.
These constructs are used like JSONata or Javascript function declaration in the sense that the value of the declaration (a function) can be
assigned to variables and used directly.
For example, \stt{\$addOne := function(x)\{x + 1\}} defines a function and assigns it to the variable \stt{\$addOne}.
\stt{\$addOne(3)} is a call to the function with the argument 3.
Similarly, \stt{\$myQuery := query()\{[\$DB1 ?person :age 42]\}} defines a query that can be used like a function.
\stt{\$myQuery(\$myDB)} is a call to the function with whatever ``database'' is assigned to \stt{\$myDB}.
However, unlike ordinary functions, the body here consists of one or more \textit{patterns} such as the pattern  \stt[\$DB1 ?person :age 42] shown. 
We'll start by talking about the argument to the query, for example, the value assigned to \stt{\$MyDB} in the expression \stt{\$myQuery(\$MyDB)} then get back to talking about the patterns. 

Relational (table-based) and graph-based data can be described by triples $[x,rel,y]$ where $x$ is an entity identifier, $y$ is data (string, number, object, array, another entity identifier, etc.) and $rel$ is a relationship (predicate) holding between $x$ and $y$.
...
\begin{figure}[H]
  \caption{Bob as a graph}
  \label{fig:bob-as-a-graph}
     \includegraphics[scale=0.15]{bob-age-42.jpg}
  \centering
\end{figure}

You might wonder where the 152 in the triples came from, or for that matter, why the fact that Bob being 42 couldn't
simply be represented by the single triple \stt{['Bob' 'age' 42]}.
The answer is that you could represent this fact with that one triple, but then you are using \stt{'Bob'} as a key
which might not be that good if your database has more than one Bob in it. 
So instead, to prepare for the more general case, Datalog-like languages use unique integers to refer to entities.
...
On Line 1 note that we put a JSON-like object in an Javascript-like array by wrapping it in square brackets.
On Line 2 we defined the query function; here note the use of a \textit{query variable} \stt{?e}.
Also note that whereas we used the string \stt{'age'} for the relation, we used \stt{:age}, syntax we call a \textit{role} used to
represent that same relation in the pattern.
...
The result of this query, assigned to \stt{\$result} is a \textit{binding set}, a set of values that bind to the query variables.
In this case, with the data in Figure~\ref{fig:bob-as-a-graph}, the binding set would be simply \stt[\{?e : 152\}].
This indicates that the entity indexed at 152 has a attribute :age with value 42.
If there were more such values entities in the database, the query would have returned one such \stt{\{?e : <whatever>\}}
binding for each of them.
We call the collection of such bindings a \textit{binding set}.
...
All we got was a binding set for every entity that has an age attribute (or role) equal to 42.
Entity IDs are internal to the database and not of much value to RADmapper users.
In order to make any use of this information, we need to join the \stt{[?e :age 42]} pattern with more information.
This is shown in Figure~\ref{code:bob-age-more} below.

\begin{figure}[H]
    \caption{A more useful query}
...
In this example, Line 2 is as it was, but now a new line, Line 3, by virtue of its use of \stt{?e} again imposes an additional
constraint on the graph match: the entity bound to \stt{?e} must have a \stt{:name} attribute.
The value of the name attribute is bound to \stt{?name}. Thus each \textit{binding element} in the binding set will
bind two variable, \stt{?e} and \stt{?name}.
The binding set for the database depicted in the graph depicted in Figure~\ref{fig:bob-as-a-graph} is \stt{[?e : 152, ?name 'Bob']}.

In the example, we used the patterns to match on the entity field; any of the entity, attribute, or value positions can be used in the pattern.
Further, you can use variables in more than one of those positions; \stt{query()\{[?entity ?attr ?val]\}} is a query that
matches on every entity attribute and value of the database.
\stt{query\{[\_ ?attr \_]\}} would return all the attributes in the database (without duplicates).
When matching the value position you are doing a relation join, for example, we might match the social security number, SSN in
some data against a same-valued (but possibly differently named) value in other data. For example,
...
You have probably noticed that the query above, and one used earlier have four element in their pattern whereas most of the examples have only three.
If four elements are provided the first is the database to search against.
This provides a segue to a broader discussion of mapping from multiple sources.
Very little of what has been presented so far is a novel invention of RADmapper;
these are ordinary features of Datalog, a language which was studied extensively in the 1980s \cite{Abiteboul1995a}.
and an influence on languages such as SPARQL and the Shapes Constraint Language (SHACL).
After discussion of mapping from multiple sources we'll discuss the higher-order version of \stt{query} and the uses of \stt{express}. 

\subsection{Matching against multiple sources}
