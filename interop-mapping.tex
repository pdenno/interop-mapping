...
keywords={query, enforce, \$MCnewContext, \$MCaddSource, \$readFile, \$reduce, typeof, new, true, false, catch, function, return, null, catch, switch, var, if, in, while, do, else, case, break},
...
\begin{figure}[H]
  \caption{Result of mapping the data depicted in Figure~\ref{code:endurant}.
    ...
    (1) DML to capture metadata for \stt{owl/Class} and \stt{owl/ObjectProperty} objects in the static tables defined above, and
    (2) DDL to create tables for \stt{owl/ObjectProperty} objects.}
...
( $mc := $MCnewContext() ~> $MCaddSource($readFile('data/testing/owl-example.edn'), 'owl-source');
  ...
  $qClass := $qtype('owl/Class');            // Use the 'template', here..
  $qProp  := $qtype('owl/ObjectProperty');   // ...and here.

  // Run the clas query; return a collection of binding sets.
  $bsets := $qClass($MCgetSource($mc, 'owl-source')); 

  $tar_data := $reduce($bsets,
                        enforce($bs)
                           {  {'instance-of'  : 'insert-row',
                               'table'        : 'ObjectDefinition',
   ...
   $bsets := $qProp($MCgetSource($mc, 'owl-source'));
   ...
   $reduce($bsets,
           ...
           $tar_data   // This time, we're updating the target data.
          )
)
\end{lstlisting}
\end{figure}
