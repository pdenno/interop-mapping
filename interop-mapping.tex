...
Given examples of the data source and target in Figure~\ref{fig:data-for-end-to-end} it is not hard to see that the function in Figure~\ref{fig:function-returned-from-shape-call} would get the job done;
...
\begin{figure}[H]
  \caption{The function returned by the call to \$shape}
...
Of course, it isn't hard to identify the mapping requirements using LLM tools either.
...
Below we show how it can be used to automatically generate functions like in Figure~\ref{fig:function-returned-from-shape-call}.
...
mapping functions like the one in Figure~\ref{fig:function-returned-from-shape-call}:
\begin{description}
  \item[Step A] Compute the abstract ``shape'' of the source and target structures in Figure~\ref{fig:data-for-end-to-end}
  \item[Step B] Provide these shapes to an LLM function, \stt{\$llmMatch}, to reconcile differences in the source and target structures, producing functions for mapping between them.
\item[Step C] Verify, document, and store the function for use whenever it is needed.
\end{description}

We will start by discussing Step C because it is a more basic capability used in Step A.
The key RADmapper functionality of Step C is to store the function (\stt{\$put});
the converse is, of course, retrieving it (\stt{\$get}).
We can demonstrate use of the stored function with the exerciser web app, in which case we'd use \stt{\$get} to get both data and the function.
...
\begin{figure}[H]
  \caption{In the three-step process, \stt{invoice-match-1->2-fn}, a mapping function,
    was created in Step B, and stored in Step C.
...
\end{lstlisting} \vspace{-2em}
Of course, you could create things like this by typing them in, but so as to illustrate the use of a RADmapper function-like construct \stt{query}, we will assume there are schema defining these structures; we'll use \stt{query} to create the structures. The code to do this is a bit involved; it would be saved as a function and applied, but for the sake of exposition we'll discuss the whole thing, shown below.
Just keep in mind that this is part of the much simpler process of doing the three steps.
...
\begin{figure}[H]
  \caption{Step A, calling the shape function, and Step B, using the shapes to create a mapping function.}
...
% THIS NEEDS WORK. I THINK I SHOULD BE TALKING ABOUT VALIDATION (Step C). <=============================
