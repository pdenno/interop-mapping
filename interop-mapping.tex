...
As the figure suggests, this target data is more verbose, and only in Hollywood do the colons line up in the columns like that.
A thought at what the \stt{express} structure for this data might be as follows.
\begin{figure}[H]
  \caption{Possible express structure for target data as depicted in Figure~\ref{data:slack-with-keys}}
...
A better alternative would be to reduce over the \stt{express}, \stt{\$reduce(\$bsets,\$ex1)}.
There is a problem in doing this with the \stt{express} body as written above.
