...
\usepackage{helvet} % like FY24 project
...
\begin{itemize}
  \item{an end-to-end mapping task,}
...
\subsection{End-to-end preview}

Without going into detailed description of how the parts work, this section describes a mapping task end-to-end.
The example is typical of ``mapping'' problem: we have data encoded in one structure (the ``source'' form) and we'd like to express that data as another structure (the ``target'' form).
...
\begin{itemize}
  ...
  \item{human validation of an AI-generated mapping function.}
  \end{itemize}

The last bullet above is in keeping with basic goals of RADmapper: we aim to produce functions that expresses the essence of a mapping task in terms that aren't so technical that business-oriented analysts can't read them.
...
A light rewrite of the RADmapper function here (as, say, JSON) can be shared and mapped to commercial tools to execute the essential activity in technology other than RADmapper.

\textbf{[Now details of the example.]}

This example is currently executable from the exerciser https://hub.docker.com/r/podenno/rm-exerciser as the default example, if you'd like to try it yourself.
%RADmapper is available for NodeJS library and Java. The exerciser has a OpenAPI interface.
