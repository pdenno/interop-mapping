...
There is a similar construct \stt{queryOne} that can be used to get a binding set for the first match found.

Primarily for use in development and debugging, there is a shortcut to getting binding sets: the function \stt{query!}.
\stt{query!} takes a collection of triple forms (and no parameters) and returns an array of binding sets matching the query.
For example:
...
(b) how do we know whether to use a pointer to reference some data rather than just put it where it is mentioned?
...
It is enough that the programmer recognize the type by the attributes it possesses.
...
The \stt{db/type} can be any primitive type defined (e.g. strings, numbers, UUID, etc.) or ``ref'', meaning that
the attribute is populated by references to other objects.
In relational parlance the refs are foreign keys. 
...
\begin{lstlisting}[numberstyle=\scriptsize,basicstyle=\ttfamily\scriptsize,numbers=left,stepnumber=1,breaklines=true]  
  [{"schema" : {"db/attrs"  : [{"schema/name"   : {"db/type" : "string", "db/cardinality" : "one"}}],
   ...
   {"table"  : {"db/attrs"  : [{"table/name"    : {"db/type" : "string", "db/cardinality" : "one" },
				"table/schema"  : {"db/type" : "ref"   , "db/cardinality" : "one" },
				"table/columns" : {"db/type" : "ref"   , "db/cardinality" : "many"}}],
   ...
   {"column" : {"db/attrs"  : [{"column/name"   : {"db/type" : "string", "db/cardinality" : "one"}},
			       ...
			       {"column/table"  : {"db/type" : "ref",    "db/cardinality" : "one"}}],
...
Keep in mind that the elements of the elements of a triple are always atomic.
To represent $n$ values there are $n$ triples having different values for the 3rd element of the triple.
...
Figure \ref{code:mapping-owl-to-rdbms} depicts the complete code for performing the mapping to target schema, tables and columns.
The \stt{where} clause sets up a query to produce a binding set for the source data.
The section in curly brackets below the triple forms define how values from the binding sets are used in the target population.
\textbf{[Note that...to be completed.]}
...
\begin{lstlisting}[numberstyle=\scriptsize,basicstyle=\ttfamily\scriptsize,numbers=left,stepnumber=1,breaklines=true]
  (where [?class rdf/type            "owl/Class"]
	 ...
	 [?rel   rdf/type            (or "owl/ObjectType" "owl/DataType")]
