...
%\usepackage{inconsolata} % https://tex.stackexchange.com/questions/50810/good-monospace-font-for-code-in-latex    % pendant
...
\usepackage[scaled=0.92]{helvet} % Sans serif font. https://mirror.las.iastate.edu/tex-archive/macros/latex/required/psnfss/psnfss2e.pdf
...
This document describes the RADmapper language and its strategy for serving as an interoperable exchange form mapping specifications.
...
The reference implementation of the RADmapper language can be found in the Github repository https://github.com/pdenno/RADmapper. %\href{https://github.com/pdenno/RADmapper}{https://github.com/pdenno/RADmapper}.
An web-based ``exerciser'' tool to explore the language is available as a Docker image https://hub.docker.com/r/podenno/rm-exerciser.
...
The example is typical of ``mapping'' problems: we have data encoded in one structure (the ``source'' form) and we'd like to express that data as another structure (the ``target'' form).
...
each InvoiceLine, as shown by the nesting of Line 34 in your target structure.}
\end{enumerate}
...
Both Line 2 and Line 3, use JSONata syntax \stt{.fn\_exe} to get the one property retrieved from the object, its executable.
Of course, "executable" means different things in different contexts; if the code is retrieve under Javascript, like with the web exerciser, the code is JS-executable;
if \stt{\$get} is called from the server, Java-executable code is provided.\footnote{You are probably wondering what ``getting the executable'' means with respect to  data.
...
returned would be the following string value:
...
\begin{itemize}
   ...
   \item{Lines 14, 18, 22, and 26 call the function \stt{\$llmExtract}, which is a LLM call to extract the information describe by the second argument string from the first argument string.}
